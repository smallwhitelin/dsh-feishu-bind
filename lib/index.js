...
export function createBindService({ config, log = () => {}, deps = {} }) {
  ...
  async function startFlow() {
    ...
    if (!registerApp) {
      try { registerApp = (await import("@larksuiteoapi/node-sdk")).registerApp; }
      catch (e) { S.phase = "error"; S.error = `缺少依赖 @larksuiteoapi/node-sdk：${e?.message || e}`; note(S.error); return; }
    }
  ...
  function requestListener(req, res) {
    ...
    try {
      if (config.token && url.searchParams.get("k") !== config.token) {
        return json(res, 403, { error: "口令不对：请在网址后带 ?k=<token>" });
      }
