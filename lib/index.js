...
import { existsSync, mkdirSync, readFileSync, readdirSync, rmSync, renameSync, symlinkSync, writeFileSync } from "node:fs";
...
export const Config = z.object({
  ...
  // === 独立实例（多个机器人并行）：新建应用时派生一套自己的环境 ===
  /** 本实例用的 dsh profile 名；派生实例复用同一份 profile（插件只维护一份） */
  profileName: z.string().default(process.env.DSH_PROFILE || "feishu"),
  /** 派生实例的 DSH_HOME 目录名：<家目录>/.<instancePrefix>-<名字> */
  instancePrefix: z.string().default("dsh"),
  /** 本实例是否启动绑定页；派生出来的实例置 0，避免抢同一个端口 */
  enabled: z.boolean().default(process.env.DSH_BIND_ENABLED !== "0"),
  /** 派生实例时以符号链接共享的项（相对 DSH_HOME）：模型配置、记忆 */
  shareFrom: z.array(z.string()).default(["settings.yaml", "memory"]),
  /** true = "新建应用"派生独立实例（多个机器人并行）；false = 覆盖当前实例的凭据 */
  independentInstances: z.boolean().default(true),
});
...
/** 把用户给的名字变成安全的实例名（只留小写字母/数字/连字符） */
export function sanitizeInstanceName(raw, fallback = "feishu2") {
  const v = String(raw ?? "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, "-")
...
/**
 * 规划一套独立实例（纯函数，不碰磁盘）：
 *   - 独立 DSH_HOME：<家目录>/.<prefix>-<名字> → 会话、飞书映射、storage 全隔离；
 *   - 独立凭据文件 <家目录>/.config/dsh-<名字>.env；
 *   - 独立 systemd 用户单元 dsh-<名字>.service；
 *   - profile（插件树）与 settings.yaml / memory 用符号链接共享 → 插件只维护一份。
 */
export function planInstance({
  ...
  sglangKey = "",
  shareFrom = ["settings.yaml", "memory"],
} = {}) {
  const unitName = `dsh-${name}.service`;
  const unitPath = join(home, ".config", "systemd", "user", unitName);
  const envFile = join(home, ".config", `dsh-${name}.env`);
  const instanceHome = join(home, `.dsh-${name}`);
  const logFile = join(logDir, `dsh-${name}.log`);
  const dirs = [
    join(instanceHome, "profiles"),
    join(instanceHome, "sessions"),
    join(instanceHome, "feishu"),
    join(instanceHome, "storages"),
    join(home, ".config", "systemd", "user"), // 单元文件目录（顺便把 .config 建出来）
    logDir,
  ];
  const links = [
    { path: join(instanceHome, "profiles", profileName), target: join(dshHome, "profiles", profileName) },
  ];
  for (const item of shareFrom) links.push({ path: join(instanceHome, item), target: join(dshHome, item) });

  const env = {
    FEISHU_APP_ID: appId,
    FEISHU_APP_SECRET: appSecret,
    FEISHU_TENANT: tenant || "feishu",
    DSH_BIND_ENABLED: "0", // 派生实例不再起绑定页，避免抢端口
    ...(sglangKey ? { SGLANG_API_KEY: sglangKey } : {}),
  };
  const envText = Object.entries(env).map(([k, v]) => `${k}=${v}`).join("\n") + "\n";
  const unitText = `[Unit]
Description=dsh bridge instance ${name} (profile: ${profileName})
...
Environment=DSH_HOME=${instanceHome}
ExecStart=${execStart}
Restart=always
RestartSec=5
KillMode=mixed
...
`;
  return { name, instanceHome, envFile, unitName, unitPath, logFile, dirs, links, env, envText, unitText };
}

/** 把规划落到磁盘并启动服务（IO 都从这里走，测试可注入 io） */
export async function provisionInstance(plan, io = {}) {
  const fsx = io.fsx || { mkdirSync, writeFileSync, rmSync, symlinkSync, existsSync };
  const exec = io.exec || ((cmd, args) => new Promise((resolve, reject) => {
  ...
  for (const d of plan.dirs) fsx.mkdirSync(d, { recursive: true });
  for (const l of plan.links) {
    try { if (fsx.existsSync(l.path)) fsx.rmSync(l.path, { recursive: true, force: true }); } catch { /* ignore */ }
    try { fsx.symlinkSync(l.target, l.path); } catch { /* 目标不存在时跳过（如还没有 memory 目录） */ }
  }
  fsx.writeFileSync(plan.envFile, plan.envText, { mode: 0o600 });
  fsx.writeFileSync(plan.unitPath, plan.unitText, { mode: 0o644 });
  await exec(SYSTEMCTL, ["--user", "daemon-reload"]);
  await exec(SYSTEMCTL, ["--user", "enable", "--now", plan.unitName]);
  return { ok: true, unitName: plan.unitName, envFile: plan.envFile, instanceHome: plan.instanceHome };
}
...
export function createBindService({ config, log = () => {}, deps = {} }) {
  ...
  const rt = { serviceState: null, forceCreateNow: null, pendingInstance: null }; // 运行时快照（不进状态文件）
  ...
  function loadState() {
    try {
      ...
      if (s?.appId) {
        S.appId = s.appId; S.tenant = s.tenant || ""; S.boundAt = s.boundAt || 0; S.instance = s.instance || null;
        S.phase = "bound";
  ...
  function saveState() {
    try {
      ...
      writeFileSync(config.stateFile, JSON.stringify({ appId: S.appId, tenant: S.tenant, boundAt: S.boundAt, instance: S.instance || null }, null, 2), "utf8");
  ...
  async function startFlow(opts = {}) {
    ...
    // 新建应用时这一轮要落到哪个独立环境：现在就定下来，页面显示与实际用的必须同一个名字
    const independent = config.independentInstances !== false; // 未显式关闭即视为开启
    rt.pendingInstance = forceCreate && independent
      ? sanitizeInstanceName(opts.instanceName || nextInstanceName(), nextInstanceName())
      : null;
    S.phase = "waiting";
    S.error = "";
    S.qrUrl = "";
    ...
    try {
      ...
      await applyBindings({ newInstance: rt.pendingInstance || undefined });
    } catch (e) {
      if (ac.signal.aborted) return;
      S.phase = "error";
  ...
  /** 下一个可用的实例名：feishu2 / feishu3 …（按已存在的 systemd 单元避开重名） */
  function nextInstanceName() {
    try {
      const dir = join(homedir(), ".config", "systemd", "user");
      const used = new Set(
        readdirSync(dir).filter((f) => /^dsh-.+\.service$/.test(f)).map((f) => f.replace(/^dsh-/, "").replace(/\.service$/, "")),
      );
      for (let i = 2; i < 100; i += 1) if (!used.has(`feishu${i}`)) return `feishu${i}`;
    } catch { /* ignore */ }
    return "feishu2";
  }

  /** 由当前实例派生的新实例规划（ExecStart 直接沿用当前单元，保持 node/路径一致） */
  function planForInstance(name) {
    let execStart = "";
    try {
      const u = readFileSync(join(homedir(), ".config", "systemd", "user", config.serviceUnit), "utf8");
      const m = /^ExecStart=(.+)$/m.exec(u);
      if (m) execStart = m[1].trim();
    } catch { /* 读不到就退回默认 */ }
    if (!execStart) execStart = `${process.execPath} ${process.argv[1] || "dsh"} --profile ${config.profileName}`;
    return planInstance({
      name,
      appId: S.appId,
      ...
      execStart,
      sglangKey: parseEnv(readEnvText()).SGLANG_API_KEY || "",
      shareFrom: config.shareFrom,
    });
  }

  async function applyBindings(opts = {}) {
    try {
      S.phase = "applying";
      if (!S.appSecret) S.appSecret = parseEnv(readEnvText()).FEISHU_APP_SECRET || "";
      if (!S.appId || !S.appSecret) throw new Error("凭据不完整，无法写入");

      // 新建应用 + 独立实例模式：派生一套自己的环境，当前实例完全不动
      if (opts.newInstance && config.independentInstances !== false) {
        const plan = planForInstance(opts.newInstance);
        note(`正在创建独立实例 ${plan.unitName}（DSH_HOME=${plan.instanceHome}，凭据 ${plan.envFile}）…`);
        await (deps.provision || provisionInstance)(plan);
        S.instance = { name: plan.name, unit: plan.unitName, envFile: plan.envFile, home: plan.instanceHome };
        S.boundAt = Date.now();
        saveState();
        S.phase = "bound";
        note(`独立实例 ${plan.unitName} 已创建并启动；当前实例未受影响`);
        return true;
      }
  ...
  function snapshot() {
    ...
    return {
      ...
      independentInstances: config.independentInstances !== false,
      instance: S.instance || null,
      nextInstanceName: nextInstanceName(),
      pendingInstance: rt.pendingInstance,
  ...
  async function requestListener(req, res) {
    ...
    try {
      ...
      if (req.method === "POST" && url.pathname === "/api/qr") {
        ...
        startFlow({ create: createNow, instanceName: want.name }); // 不 await：先回包，页面轮询拿二维码
      ...
      if (req.method === "POST" && url.pathname === "/api/apply") {
        // 上一轮若是"新建应用"，重试仍走同一套独立实例，绝不覆盖当前实例
        return applyBindings({ newInstance: rt.pendingInstance || undefined })
...
export function apply(ctx, config) {
  if (!config.enabled) {
    try { ctx.logger?.info?.("[feishu-bind] 本实例已禁用绑定页（DSH_BIND_ENABLED=0）"); } catch { /* ignore */ }
    return null;
  }
...
<div class="card">
  ...
  <div id="actions"></div>
  <div id="newbox" class="small" style="display:none;margin-top:16px;padding-top:14px;border-top:1px dashed var(--line)">
    再要一个<strong>独立的机器人</strong>（自己的会话与环境，和当前这个并行跑）<br>
    环境名 <input id="instname" value="" maxlength="20"
      style="width:112px;padding:7px 9px;font-size:14px;border:1px solid var(--line);border-radius:9px;text-align:center" />
    <a href="#" id="newapp" style="color:var(--brand);font-size:13.5px">用新应用绑定</a>
    <div class="small" id="newhint" style="margin-top:6px"></div>
  </div>
...
function render(s) {
  ...
  if (s.phase === 'bound') {
    if (s.wired) {
      stage.innerHTML = '<div class="tick hint" title="点我生成新二维码">✓</div>'
        + '<div class="status ok">' + (s.instance ? '独立实例已创建并启动' : '已绑定，桥已接线') + '</div>'
        + '<div class="small">' + (s.instance
            ? '环境：<code>' + esc(s.instance.unit) + '</code>　凭据：<code>' + esc(s.instance.envFile) + '</code><br>它和当前机器人并行运行，各自独立会话。'
            : '点上面的绿勾可生成新二维码（扫码即替换绑定）') + '</div>';
    ...
    if (s.pendingInstance) {
      actions.innerHTML = '<button class="primary" id="retry2">重试创建环境 ' + esc(s.pendingInstance) + '</button>'
        + '<div class="small" style="margin-top:8px">不用重新扫码：直接用刚才授权的结果创建 <code>dsh-' + esc(s.pendingInstance) + '.service</code></div>';
      document.getElementById('retry2').onclick = async () => {
        document.getElementById('stage').innerHTML = '<div class="spinner"></div><div class="status">正在重试…</div>';
        try { await api('/api/apply', 'POST'); } catch (e) { /* 轮询会兜底 */ }
        tick();
      };
    } else {
    ...
    stage.innerHTML = '<div class="qr"><img alt="二维码" src="' + s.qrPng + '"></div>'
      ...
      + '<div class="small">' + fmtLeft(s.expiresAt, s.serverTime) + (s.forceCreate
        ? (s.pendingInstance ? '　（将新建应用 → 独立环境 <b>' + esc(s.pendingInstance) + '</b>）' : '　（将新建应用）')
        : (s.envAppId ? '　（将重新授权既有应用 ' + esc(String(s.envAppId).slice(0,14)) + '…）' : '　（将新建应用）')) + '</div>';
  ...
  // 新建独立环境那一块：固定 DOM，只在等待扫码时显示；输入框内容始终与"这一轮实际用的名字"一致
  const newbox = document.getElementById('newbox');
  const nameInput = document.getElementById('instname');
  const hint = document.getElementById('newhint');
  const showNew = s.independentInstances && s.phase === 'waiting';
  newbox.style.display = showNew ? '' : 'none';
  if (showNew) {
    const want = s.forceCreate ? (s.pendingInstance || s.nextInstanceName) : (s.nextInstanceName || 'feishu2');
    if (document.activeElement !== nameInput) nameInput.value = want;
    hint.innerHTML = s.forceCreate && s.pendingInstance
      ? '这一轮会创建独立环境 <b>' + esc(s.pendingInstance) + '</b>（服务 dsh-' + esc(s.pendingInstance) + '.service）'
      : '填好名字后点右边链接，生成的二维码就是给这个新环境的';
  }
  ...
  if (s.instance) rows.push(['本页新建的实例', s.instance.unit + '（' + s.instance.home + '）']);
  if (s.appSecret) rows.push(['app secret', s.appSecret]);
  meta.innerHTML = rows.map(([k, v]) => '<div><b>' + esc(k) + '</b><span>' + esc(v) + '</span></div>').join('');
  meta.style.display = '';
}

async function newQr(opts) {
  if (busy) return; busy = true;
  const body = opts === true ? { create: true } : (opts && typeof opts === 'object' ? opts : {});
...
let nameTimer = null;
document.getElementById('instname').addEventListener('input', () => {
  clearTimeout(nameTimer);
  nameTimer = setTimeout(async () => {
    const st = await api('/api/state');
    // 只有在"新建独立环境"这一轮里才需要跟着改名重出码；改名即重新生成，保证显示与实际一致
    if (st.forceCreate) newQr({ create: true, name: document.getElementById('instname').value });
  }, 500);
});
document.getElementById('newapp').addEventListener('click', (e) => {
  e.preventDefault();
  newQr({ create: true, name: document.getElementById('instname').value });
});
