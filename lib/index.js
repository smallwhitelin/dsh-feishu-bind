...
function render(s) {
  ...
  if (s.phase === 'bound') {
    ...
    actions.innerHTML = '<button id="again">换一张二维码</button>'
      + (s.independentInstances
        ? '<div class="small" style="margin-top:14px">想再要一个<strong>独立的机器人</strong>（独立会话/环境，和现在这个并行跑）？<br>'
          + '点下面按钮 → 扫码新建应用 → 自动起一套独立环境<br>'
          + '<span class="small">环境名 <input id="instname" value="' + esc(s.nextInstanceName || "feishu2") + '" '
          + 'style="width:110px;padding:6px 8px;font-size:13px;border:1px solid #e5e7eb;border-radius:8px;text-align:center" /> '
          + '<a href="#" id="newapp" style="color:#2563eb;font-size:13px">用新应用绑定</a></span></div>'
        : '');
    document.getElementById('again').onclick = () => newQr();
    const na = document.getElementById('newapp');
    if (na) na.onclick = (e) => {
      e.preventDefault();
      const nm = (document.getElementById('instname') || {}).value || '';
      newQr({ create: true, name: nm });
    };
  } else {
