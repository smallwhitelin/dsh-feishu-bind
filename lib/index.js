...
export const Config = z.object({
  ...
  /**
   * 访问口令。默认 `dsh`（开箱即有一道门），**请改成你自己的**；
   * 设成空串则不需要口令。校验方式：页面输入口令（登录后 Cookie 记 30 天）
   * 或访问时带 `?k=<token>`。
   */
  token: z.string().default("dsh"),
...
export function createBindService({ config, log = () => {}, deps = {} }) {
  ...
  const rt = { serviceState: null }; // 运行时快照（不进状态文件）
  ...
  async function startFlow() {
    ...
    const existing = config.forceCreate ? "" : (config.appId || parseEnv(readEnvText()).FEISHU_APP_ID || "");
  ...
  function snapshot() {
    ...
    return {
      ...
      forceCreate: Boolean(config.forceCreate),
  ...
  async function requestListener(req, res) {
    ...
    try {
      ...
      if (req.method === "POST" && url.pathname === "/api/qr") {
        startFlow(); // 不 await：先回包，页面轮询拿二维码
        return json(res, 200, { ok: true, phase: "waiting", hasExistingApp: Boolean(config.forceCreate ? "" : (config.appId || parseEnv(readEnvText()).FEISHU_APP_ID)) });
      }
...
<style>
  ...
  * { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  body { margin:0; min-height:100vh; display:flex; align-items:center; justify-content:center;
         background:#f6f7f9; color:var(--fg); font:16px/1.55 -apple-system,"PingFang SC","Microsoft YaHei",system-ui,sans-serif; padding:20px; }
  ...
  .qr { width:264px; height:264px; margin:6px auto 10px; display:flex; align-items:center; justify-content:center;
...
function render(s) {
  ...
  if (s.phase === 'bound') {
    ...
    stage.innerHTML = '<div class="qr"><img alt="二维码" src="' + s.qrPng + '"></div>'
      ...
      + '<div class="small">' + fmtLeft(s.expiresAt, s.serverTime) + (s.envAppId ? '　（扫码将重新授权既有应用）' : '　（扫码将新建应用）') + '</div>';
...
async function newQr() {
  ...
  try { await api('/api/qr', 'POST'); } catch (e) { /* 轮询会兜底 */ }
  busy = false;
  tick();
}
...
<style>
  body { margin:0; min-height:100vh; display:flex; align-items:center; justify-content:center; background:#f6f7f9;
         color:#12141a; font:16px/1.55 -apple-system,"PingFang SC","Microsoft YaHei",system-ui,sans-serif; padding:20px; }
  ...
  input { width:100%; padding:12px 14px; font-size:15px; border:1px solid #e5e7eb; border-radius:11px; outline:none; }
  input:focus { border-color:#2563eb; box-shadow:0 0 0 3px rgba(37,99,235,.12); }
  button { margin-top:14px; width:100%; padding:12px 18px; font-size:15px; border-radius:11px; border:0; background:#2563eb; color:#fff; cursor:pointer; }
...
<div class="card">
  <h1>飞书机器人绑定</h1>
  <p>这个页面需要一个口令才能打开</p>
  <input id="k" type="password" placeholder="请输入口令" autocomplete="current-password" autofocus>
