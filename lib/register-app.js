...
export async function registerApp({ domain, larkDomain, source, signal, onQRCodeReady, onStatusChange, appPreset, addons, createOnly, appId }, deps = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    let settled = false;
    let pollTimer = null;
    let expireTimer = null; // 必须与轮询定时器一起清理，否则会留下长定时器把进程钉住
    const setTimer = deps.setTimeout || setTimeout;

    const cleanup = () => {
      if (pollTimer !== null) { clearTimeout(pollTimer); pollTimer = null; }
      if (expireTimer !== null) { clearTimeout(expireTimer); expireTimer = null; }
    ...
    expireTimer = setTimer(
      () => fail(Object.assign(new Error("二维码已过期"), { code: "expired_token" })),
      (beginRes.expires_in ?? 600) * 1000,
    );
